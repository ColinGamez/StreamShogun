    ...
    try {
      ...
      if (settings && Object.keys(settings).length > 0) {
        ...
        for (const [key, value] of Object.entries(settings)) {
          bridge.dbSetSetting(key, value).catch(() => {
            /* best-effort */
          });
        }
      }
      ...
      if (history && history.length > 0) {
        ...
        const merged = [...byKey.values()].sort((a, b) => b.watchedAt - a.watchedAt).slice(0, 50);
  ...
  cloudPush: async () => {
    ...
    try {
      ...
      const localUpdatedAt =
        lastAt > 0 ? new Date(lastAt).toISOString() : new Date(0).toISOString();
