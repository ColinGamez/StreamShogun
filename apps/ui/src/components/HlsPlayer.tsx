...
export function HlsPlayer({
  ...
  const [volume, setVolume] = useState(() => {
    try {
      return parseFloat(localStorage.getItem("shogun:volume") ?? "0.8");
    } catch {
      return 0.8;
    }
  });
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("shogun:volume", String(volume));
    } catch {
      /* */
    }
  ...
  useEffect(() => {
    ...
    const onWaiting = () => {
      ...
      stallTimer = setTimeout(() => {
        ...
        const hls = hlsRef.current;
        if (hls) {
          console.warn("[HlsPlayer] Buffer stall – recovering");
  ...
  return (
    ...
    >
      ...
      <div className={`hls-overlay${overlayVisible || buffering || errorMsg ? " visible" : ""}`}>
        ...
        {/* Error message */}
        {errorMsg && (
          <div className="hls-error-banner" role="alert">
            {errorMsg}
          </div>
        )}
