...
export function BillingStateBanner() {
  ...
  const periodEndStr = currentPeriodEnd ? new Date(currentPeriodEnd).toLocaleDateString() : null;
    ...
    return (
      <div className="billing-banner billing-banner-warning" role="alert">
        ...
        <span className="billing-banner-text">
          ...
          <button className="link-btn" onClick={() => bridge.billingPortal()}>
  ...
  if (subscriptionStatus === "CANCELED" && authPlan === "PRO" && periodEndStr) {
    return (
      <div className="billing-banner billing-banner-info" role="status">
        <span className="billing-banner-icon">ℹ️</span>
        <span className="billing-banner-text">
          Your subscription was canceled. Pro access continues until <strong>{periodEndStr}</strong>
          .
        </span>
      </div>
    );
  ...
  if (subscriptionStatus === "TRIALING" && trialEndsAt) {
    ...
    return (
      <div className="billing-banner billing-banner-trial" role="status">
        ...
        <span className="billing-banner-text">
          Free trial — Pro features active until <strong>{trialEndStr}</strong>. Cancel anytime.
        </span>
      </div>
    );
  }
