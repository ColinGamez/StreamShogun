...
describe("buildSupportBundle", () => {
  ...
  it("redacts JWT tokens in messages", () => {
    ...
    const msgs = [makeMsg("user", `token is ${fakeJwt}`)];
    const bundle = buildSupportBundle(baseCtx({ messages: msgs }));
  ...
  it("redacts Stripe keys in messages", () => {
    const msgs = [makeMsg("user", `key is sk_${"test"}_abc1234567890xyz`)];
    const bundle = buildSupportBundle(baseCtx({ messages: msgs }));
    expect(bundle.recentMessages[0].text).not.toContain(`sk_${"test"}_`);
  ...
  it("does not contain auth tokens anywhere in the serialised bundle", () => {
    const tokenPrefix = ["eyJ", "fakeToken"].join("");
    const msgs = [makeMsg("user", `token Bearer ${tokenPrefix}.abc.xyz`)];
    const bundle = buildSupportBundle(baseCtx({ messages: msgs }));
    const json = JSON.stringify(bundle);
