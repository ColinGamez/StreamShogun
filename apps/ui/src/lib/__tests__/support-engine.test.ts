...
describe("formatCitation", () => {
  it("formats citation with heading", () => {
    expect(formatCitation("playback-troubleshooting", "Black Screen")).toBe(
      "[guide:playback-troubleshooting#Black Screen]",
    );
  });

  it("formats citation without heading", () => {
    expect(formatCitation("adding-playlists")).toBe("[guide:adding-playlists]");
  });
});
...
describe("composeAnswer", () => {
  ...
  it("returns low confidence with related guides when score < 8", () => {
    mockSearchSupport.mockReturnValue([makeMatch(ARTICLE_PLAYBACK, 5), makeMatch(ARTICLE_EPG, 3)]);
  ...
  it("returns medium confidence with structured answer for score 8-14", () => {
    mockSearchSupport.mockReturnValue([makeMatch(ARTICLE_PLAYBACK, 12), makeMatch(ARTICLE_EPG, 5)]);
...
describe("renderAnswerText", () => {
  const baseAnswer: StructuredAnswer = {
    ...
    steps: [{ text: "Step one", citation: "[guide:test-article#First]" }, { text: "Step two" }],
