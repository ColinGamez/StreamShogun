...
export async function dbSetSetting(key: string, value: string): Promise<IpcResponse<null>> {
...
export function onRefreshCompleted(callback: (payload: unknown) => void): () => void {
...
export async function pipOpen(channelUrl: string, channelName: string): Promise<IpcResponse<null>> {
...
export async function discordSetActivity(
  ...
  if (hasBridge()) return window.shogun!.discordSetActivity({ details, state, startTimestamp });
  return NO_BRIDGE;
}
...
export async function featuresFetch(): Promise<
  IpcResponse<{
    plan: string;
