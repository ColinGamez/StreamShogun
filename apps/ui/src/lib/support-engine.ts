...
const CLARIFYING_QUESTIONS: Record<string, string> = {
  playback:
    "Could you describe the issue in more detail — for example, do you see a black screen, buffering, or an error message?",
...
function buildTroubleshooting(article: SupportArticle, matches: SupportSearchMatch[]): string[] {
  const tips: string[] = [];
...
function buildRelatedGuides(matches: SupportSearchMatch[], primaryId: string): RelatedGuide[] {
  return matches
...
export function composeAnswer(query: string): StructuredAnswer {
  ...
  if (confidence === "low") {
    ...
    return {
      summary: `I found some guides that might be related, but I'm not fully confident in the match.`,
      steps: [],
      troubleshooting: [],
