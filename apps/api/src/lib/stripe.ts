...
const STRIPE_LIVE_SECRET_PREFIX = `sk_${"live"}_`;

function assertStripeKeyMatchesEnv(key: string): void {
  const isLiveKey = key.startsWith(STRIPE_LIVE_SECRET_PREFIX);
  const isProduction = env.NODE_ENV === "production";

  if (isLiveKey && !isProduction) {
    throw new Error(
      `FATAL: Stripe live key detected in NODE_ENV="${env.NODE_ENV}". ` +
        "Use Stripe test keys outside production. Aborting.",
    );
  }

  if (!isLiveKey && isProduction) {
    console.warn("⚠️  Stripe TEST key in production — billing will use Stripe test mode.");
  }
}
