...
export async function adminAuth(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  if (!env.ADMIN_KEY) {
    reply.code(501).send({ error: "NotImplemented", message: "Admin API is not enabled" });
    return;
  }
  ...
  if (!key || typeof key !== "string" || !safeEqual(key, env.ADMIN_KEY)) {
    request.log.warn({ reqId: request.id, path: request.url }, "admin.auth_rejected");
    reply.code(401).send({ error: "Unauthorized", message: "Invalid or missing admin key" });
    return;
  }
}
