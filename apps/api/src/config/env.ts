...
const envSchema = z
  .object({
    ...
    // Cookie domain — set to ".streamshogun.com" in production so cookies
    // are shared between api.streamshogun.com and streamshogun.com
    ...
    // Google Sign-In (public client ID used by web sign-in + server-side token checks)
    ...
    // Roku Pay (optional — enables Roku purchase validation)
    ROKU_PAY_API_KEY: z.string().min(1).optional(),
    ROKU_PRODUCT_ID_PRO_MONTHLY: z.string().min(1).optional(),
    ROKU_PRODUCT_ID_PRO_YEARLY: z.string().min(1).optional(),
    ...
    if (data.STRIPE_SECRET_KEY && data.BILLING_DISABLED !== "true") {
      ...
      if (!data.STRIPE_PRICE_ID_PRO_MONTHLY && !data.STRIPE_PRICE_ID_PRO_YEARLY) {
        ctx.addIssue({
          ...
          message:
            "At least one of STRIPE_PRICE_ID_PRO_MONTHLY or STRIPE_PRICE_ID_PRO_YEARLY is required when billing is enabled",
        });
      }
    }
  });
