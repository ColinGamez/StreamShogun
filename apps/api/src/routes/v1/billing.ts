...
/**
 * Billing registers a raw JSON parser so Stripe webhook signatures can be
 * verified against the original request body. Non-webhook JSON routes in this
 * plugin can therefore receive a Buffer and need to decode it explicitly.
 */
function decodeJsonBody(body: unknown): unknown {
...
export async function billingRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Validate body
      const parsed = checkoutBody.safeParse(decodeJsonBody(request.body));
      if (!parsed.success) {
      ...
      if (!priceId) {
        return reply.code(501).send({
          ...
          message: `Price not configured for ${interval} interval`,
        });
      }
  ...
  app.post("/webhook", async (request: FastifyRequest, reply: FastifyReply) => {
    if (!env.STRIPE_SECRET_KEY || !env.STRIPE_WEBHOOK_SECRET) {
      return reply.code(501).send({ error: "NotImplemented", message: "Webhooks not configured" });
    }
    ...
    if (billingDisabled) {
      ...
      request.log.info(
        { ...wCtx, outcome: "ignored", reason: "billing_disabled" },
        "webhook.billing_disabled",
      );
    ...
    if (prior) {
      ...
      await prisma.webhookEvent.delete({ where: { id: prior.id } }).catch(() => {
        /* retry cleanup — non-fatal */
      });
    }
    ...
    try {
      switch (event.type) {
        ...
        default:
          ...
          await prisma.webhookEvent
            ...
            .catch((err) => {
              request.log.warn({ err }, "webhook: failed to record ignored event");
            });
      }
    } catch (err) {
      const sanitized = sanitizeError(err);
      request.log.error({ ...wCtx, outcome: "failed", err: sanitized }, "webhook.handler_error");
      ...
      await prisma.webhookEvent
        ...
        .catch((dbErr) => {
          request.log.warn({ err: dbErr }, "webhook: failed to record handler error");
        });
...
async function recordAndReturn(
  ...
  await prisma.webhookEvent
    ...
    .catch((err) => {
      console.warn("webhook: recordAndReturn update failed", err);
    });
  return result;
}
...
async function handleCheckoutCompleted(
  ...
  if (!stripeSubscriptionId) {
    ...
    return recordAndReturn(webhookEventId, {
      outcome: "ignored",
  ...
  // Fetch subscription details from Stripe outside the transaction to avoid long row locks.
  let billingInterval: "MONTHLY" | "YEARLY" | null = null;
  let mappedStatus: "ACTIVE" | "TRIALING" | "PAST_DUE" | "CANCELED" = "ACTIVE";
  ...
  try {
    ...
    log.warn({ ...wCtx }, "webhook.checkout: could not retrieve subscription details");
  }
  ...
  return prisma.$transaction<HandlerResult>(async (tx) => {
    ...
    if (existing.stripeCustomerId && customerId && existing.stripeCustomerId !== customerId) {
      log.warn({ ...wCtx, reason: "customer_mismatch" }, "webhook.checkout: customer ID mismatch");
      await tx.webhookEvent.update({
        where: { id: webhookEventId },
...
async function handleSubscriptionUpsert(
  ...
  if (isIncompleteStatus(sub.status)) {
    ...
    return recordAndReturn(webhookEventId, {
      outcome: "ignored",
      reason: `incomplete_status:${sub.status}`,
    });
  }
