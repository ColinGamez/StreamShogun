...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.get("/users", async (request, reply) => {
    const { skip, take, page, pageSize } = pagination(request.query as Record<string, unknown>);
  ...
  app.put("/feature-flags", async (request, reply) => {
    ...
    request.log.info({ userId, key, enabled, flagId: flag.id }, "admin.feature_flag_set");
