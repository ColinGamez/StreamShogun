...
import { initScheduler, setRefreshInterval, triggerRefresh, getRefreshStatus } from "./scheduler";
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IpcChannels.LICENSE_SET_KEY, (_event, args: { key: string }) => {
    try {
  ...
  ipcMain.handle(IpcChannels.LICENSE_SET_PRO_ENABLED, (_event, args: { enabled: boolean }) => {
    try {
    ...
    async (_event, args: { email: string; password: string }) => {
      try {
        ...
        if (!result.ok) {
          ...
          return fail(
            new Error(
              (body && typeof body.message === "string" ? body.message : null) ?? "Login failed",
            ),
          );
        }
        return ok(result.data);
