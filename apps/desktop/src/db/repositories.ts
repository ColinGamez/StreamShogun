...
export function getLastWatched(): WatchHistoryRow | null {
  return (
    (getDb().prepare("SELECT * FROM watch_history ORDER BY startedAt DESC LIMIT 1").get() as
      | WatchHistoryRow
      | undefined) ?? null
  );
}
...
export function getLicenseStatus(): LicenseStatus {
  ...
  const rows = db
    .prepare(
      "SELECT key, value FROM settings WHERE key IN ('isProEnabled', 'licenseKey', 'licenseValidationState')",
    )
  ...
  return {
    ...
    validationState:
      (map.licenseValidationState as LicenseValidationState) ??
      DEFAULT_LICENSE_STATUS.validationState,
  };
}
