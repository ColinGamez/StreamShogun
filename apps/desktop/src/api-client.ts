...
// Default API base URL — override via env or in-app settings
const DEFAULT_API_URL = process.env.STREAM_SHOGUN_API_URL?.trim() || "https://api.streamshogun.com";
...
export async function apiLogout(): Promise<void> {
  ...
  if (tokens?.refreshToken) {
    await apiFetch("/v1/auth/logout", {
      ...
      /* swallow – fire-and-forget logout */
    });
  }
  await clearTokens();
}
...
export async function apiBillingCheckout(interval: "monthly" | "yearly" = "monthly"): Promise<{
  ok: boolean;
  status: number;
  data: { url: string };
}> {
