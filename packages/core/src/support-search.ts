...
export class SupportCodexIndex {
  ...
  build(articles: SupportArticle[]): void {
    this.entries = articles.map((article) => {
      ...
      const tagTokens = new Set(article.meta.tags.flatMap((t) => tokenize(t)));
      const headingTokens = new Set(article.headings.flatMap((h) => tokenize(h.text)));
  ...
  search(query: string, maxResults = 5): SupportSearchMatch[] {
    ...
    scored.sort(
      (a, b) =>
        b.score - a.score || a.entry.article.meta.title.localeCompare(b.entry.article.meta.title),
    );
