...
MASTER_KOREA_EPG_URL=<optional epg2xml XMLTV output URL>
```
...
The Japan Master guide is generated from `bangumi.org` and cached by the API. For Korea, point `MASTER_KOREA_EPG_URL` at an XMLTV file generated by epg2xml or an epg2xml-compatible source; the API keeps that URL private and the desktop app loads it through the authenticated Master bridge.
