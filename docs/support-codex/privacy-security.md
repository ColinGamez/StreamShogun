...
StreamShōgun does not send app analytics or usage telemetry by default.

- App open count for upgrade nudges is stored locally on your device.
- Server-side errors may be reported through Sentry when configured, without playlist URLs, stream URLs, or payment credentials.

You can use StreamShōgun without an account for zero server-side app data collection.
