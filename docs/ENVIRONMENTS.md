...
> Historical note: this file documents the original Railway setup.
> For the current low-friction production move, use [RENDER_SETUP.md](RENDER_SETUP.md).
...
| `STRIPE_SECRET_KEY`           | `<test Stripe secret key>`                       | `<test Stripe secret key>`                   | `<live Stripe secret key>`                      |
| `STRIPE_WEBHOOK_SECRET`       | `<Stripe webhook secret>` (from `stripe listen`) | `<Stripe webhook secret>` (staging endpoint) | `<Stripe webhook secret>` (production endpoint) |
...
| `NODE_ENV`    | `live Stripe secret keys` allowed? | `test Stripe secret keys` allowed? |
| ------------- | ---------------------------------- | ---------------------------------- |
...
FATAL: Stripe live key detected in NODE_ENV="staging".
       Use test Stripe secret keys keys outside production. Aborting.
```
...
function assertStripeKeyMatchesEnv(key: string): void {
  const isLiveKey = key.startsWith("live Stripe secret key prefix");
  const isProduction = env.NODE_ENV === "production";

  if (isLiveKey && !isProduction) {
    throw new Error(`FATAL: Stripe live key detected in NODE_ENV="${env.NODE_ENV}".`);
  }
}
```
...
1. Use **Test mode** keys (`<test Stripe secret key>`, `<test Stripe publishable key>`).
...
4. Copy `<Stripe webhook secret>` → Railway staging `STRIPE_WEBHOOK_SECRET`.
5. Create test Prices (monthly + yearly) → staging `STRIPE_PRICE_ID_PRO_MONTHLY` / `STRIPE_PRICE_ID_PRO_YEARLY`.

### Production

1. Use **Live mode** keys (`<live Stripe secret key>`, `<live Stripe publishable key>`).
2. Create a webhook endpoint → `https://api-production-….up.railway.app/v1/billing/webhook`
3. Same 6 events.
4. Copy `<Stripe webhook secret>` → Railway production `STRIPE_WEBHOOK_SECRET`.
